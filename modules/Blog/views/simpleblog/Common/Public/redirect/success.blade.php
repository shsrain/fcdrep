...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>{{ trans('Home/message.jump_title') }}</title>
        <style type="text/css">
            *{ padding: 0; margin: 0; }
    ...
    <body>
        <div class="system-message">
            <h1>{{ trans('Home/message.success_title') }}</h1>
            <p class="error">{{ $message }}</p>
            <p class="detail"></p>
            <p class="jump">
                <b id="wait">{{ $waitSecond }}</b> {{ trans('Home/message.wait_message') }}
            </p>
            <div>
                <a id="href" id="btn-now" href="{{ $jumpUrl }}">{{ trans('Home/message.btn_jump') }}</a>
                <button id="btn-stop" type="button" onclick="stop()">{{ trans('Home/message.btn_stopjump') }}</button>
            </div>
        </div>
